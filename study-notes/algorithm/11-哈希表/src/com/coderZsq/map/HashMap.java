...
public class HashMap<K, V> implements Map<K, V> {
    ...
    public V put(K key, V value) {
        ...
        if (root == null) {
            root = createdNode(key, value, null);
            table[index] = root;
            size++;
            fixAfterPut(root);
            return null;
        ...
        // 看看插入到父节点的哪个位置
        Node<K, V> newNode = createdNode(key, value, parent);
        if (cmp > 0) {
    ...
    protected Node<K, V> createdNode(K key, V value, Node<K, V> parent) {
...
//            if (result != 0) return result;
    ...
    private void rotateLeft(Node<K, V> grand) {
        ...
        grand.right = child;
        parent.left = grand;
        fixAfterRotate(grand, parent, child);
    }
    ...
    private void fixAfterRotate(Node<K, V> grand, Node<K, V> parent, Node<K, V> child) {
        // 让parent成为子树的根节点
