 ...
 * <li>EmbeddedValueResolverAware's {@code setEmbeddedValueResolver}	获取 properties 文件属性值
 ...
 * @since 13 April 2001
 *
 * IOC Bean 工厂的顶级接口 定义一系列对 bean 的操作
 *
...
public interface BeanFactory {
	 ...
	 *
	 * 名称(有可能是bean的真正的名称 也有可能是工厂bean的名称 也有可能是bean的别名)
	 * @param name the name of the bean to retrieve
	 *
	 * 返回 bean 实例
	 * @return an instance of the bean
	 *
	 * 抛出的异常，容器中没有该 bean 的定义
	 ...
	 * @param name the name of the bean to retrieve
	 *
	 * 去容器中获取Bean的class类型 可以是实现的接口和父类
	 ...
	 * @param name the name of the bean to retrieve
	 *
	 * 这个是用于指定传入获取 bean 的构造函数的参数, 通过传入该参数，那么就可以明确的知道去调用哪个构造函数是实例化对象
	...
	/**
	 * 通过指定的 bean 的类型去容器中获取对象 若容器中有多个想同类型的 bean 我们通过 ctx.getBean(beanType.class) 就会抛出异常
	 *
	 ...
	 * @param requiredType type the bean must match; can be an interface or superclass
	 *
	 * 构造函数的参数的类型，在实例化的过程中不需要去推断构造函数了
	 * @param args arguments to use when creating a bean instance using explicit arguments
	 * (only applied when creating a new instance as opposed to retrieving an existing one)
	 *
	 * Bean 的实例
	...
	/**
	 * 通过 beanName 获取对应 bean 的 class 类型
	 *
	 * Determine the type of the bean with the given name. More specifically,
	 * determine the type of object that {@link #getBean} would return for the given name.
	 * <p>For a {@link FactoryBean}, return the type of object that the FactoryBean creates,
