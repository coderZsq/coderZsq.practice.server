...
import org.apache.rocketmq.spring.support.TransactionListener;
...
public class RocketMQTemplate extends AbstractMessageSendingTemplate<String>
    ...
    /**
     * <p>
     * Send message in synchronous mode. This method returns only when the sending procedure totally completes. Reliable
     * synchronous transmission is used in extensive scenes, such as important notification messages, SMS notification,
    ...
    public SendResult syncSend(String destination, Message<?> message, long timeout, int delayLevel) {
        ...
        try {
            ...
            org.apache.rocketmq.common.message.Message rocketMsg =
                RocketMQUtil.convertToRocketMessage(objectMapper, charset, destination, message);
            if (delayLevel > 0) {
     ...
     * <p>
     * Similar to {@link #syncSend(String, Object)}, internal implementation would potentially retry up to
     * {@link DefaultMQProducer#getRetryTimesWhenSendAsyncFailed} times before claiming sending failure, which may yield
    ...
    /**
     * Same to {@link #asyncSendOrderly(String, Message, String, SendCallback)} with send timeout specified in addition.
     *
     * @param destination formats: `topicName:tags`
     * @param message {@link org.springframework.messaging.Message}
     * @param hashKey use this key to select queue. for example: orderId, productId ...
    ...
    public TransactionSendResult sendMessageInTransaction(final String txProducerGroup, final String destination,
        ...
        try {
            ...
            org.apache.rocketmq.common.message.Message rocketMsg =
                RocketMQUtil.convertToRocketMessage(objectMapper, charset, destination, message);
            Object obj = txProducer.getTransactionCheckListener();
            if (obj instanceof TransactionListener) {
                TransactionListener listener = (TransactionListener)obj;
                return txProducer.sendMessageInTransaction(rocketMsg, listener, arg);
            } else {
                throw new MessagingException(
                    "TransactionMQProducer transactionCheckListener must use TransactionListener,but is "
    ...
    private TransactionMQProducer createTransactionMQProducer(String name,
        ...
        txProducer.setTransactionCheckListener(RocketMQUtil.convert(transactionListener));
