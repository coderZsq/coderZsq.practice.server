...
1. 一个是**init**()，这个方法是在我们给某个函数 decorate 时被调用，所以，需要有一个 fn 的参数，也就是被 decorate 的函数。
...
注意：上面这个示例中 decorator 类不是真正的 decorator，其中也没有**call**()，并且，wrapper 返回了原函数。所以，原函数没有发生任何变化。
