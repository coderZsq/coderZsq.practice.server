		...
		for index, vout := range tx.Vouts {
			if vout.UnLockScriptPubkeyWithAddress(address) {
				...
				if len(spentTXOutputs) != 0 {
					...
					if isUtxoTx == false {
						// 说当前交易所有与address相关的output都是UTXO
						utxo := &UTXO{tx.TxHash, index, vout}
						unUTXOS = append(unUTXOS, utxo)
					}
				} else {
...
// FindAllSpentOutputs 查找整条区块链所有已花费输出
		...
		WorkOutLoop:
			for index, vout := range tx.Vouts {
				...
				if len(txInputs) > 0 {
					...
					// 没有input引用该交易的输出, 则代表当前交易中所有的输出都是UTXO输出
					txOutputs.TxOutputs = append(txOutputs.TxOutputs, vout)
				}
			}
...
func (blockchain *BlockChain) GetHeight() int64 {
	return blockchain.Iterator().Next().Height
}

// GetBlockHashes 获取区块链所有的区块哈希
...
func (blockchain *BlockChain) GetBlock(hash []byte) []byte {
	var blockByte []byte
	blockchain.DB.View(func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte(blockTableName))
		if nil != b {
